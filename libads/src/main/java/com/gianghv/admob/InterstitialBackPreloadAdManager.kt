...
class InterstitialBackPreloadAdManager constructor(
    private val context: Context,
    val mIdAdsFull02: String

) {
    companion object {
        var isAdsLoading = false
        var isShowingAds = false
        const val FOUR_HOUR = 4 * 60 * 1000 * 60
    }
    ...
    private fun requestAdsPrepare(
        ...
        if (handler == null || isAdsLoading || mInterstitialAd != null) {
            return
        }
    ...
    fun canLoadInterBack():Boolean{
        if (isAdsLoading||isShowingAds){
            return false
        }
        return true
    }
